...
class MolCLRFineTuner:
    ...
    def train(self, random_seed):
        ...
        for epoch in range(self.config['epochs']):
            ...
            print(f"Epoch {epoch}, Train Loss: {train_loss / len(self.train_loader):.6f}")
            ...
            if epoch % self.config['eval_every_n_epochs'] == 0:
                ...
                print(f"Epoch {epoch}, Valid Loss: {valid_loss:.6f}, Valid RMSE: {valid_rmse:.6f}")
...
"""
    ...
    for random_seed in (42, 117, 709, 1701, 9001):
        ...
        for benchmark_name in moleculeace_benchmarks:
            ...
            # Create datasets
            train_dataset = MolDataset(train_df.iloc[train_idx], smiles_column=smiles_col, target_column=target_col)
            val_dataset = MolDataset(train_df.iloc[val_idx], smiles_column=smiles_col, target_column=target_col)
            test_dataset = MolDataset(test_df, smiles_column=smiles_col, target_column=target_col)
            ...
            predictions, labels, test_rmse, test_mae = finetuner.evaluate(model, test_loader)
            ...
            if 'cliff_mol' in test_df.columns:
                ...
                if len(cliff_df) > 0:
                    cliff_rmse = calculate_rmse(cliff_df[target_col], cliff_df['predictions'])
                
                if len(noncliff_df) > 0:
                    noncliff_rmse = calculate_rmse(noncliff_df[target_col], noncliff_df['predictions'])
            else:
    ...
    output_file.close()
    print(f"Training and evaluation complete. Results saved to {output_dir / 'train_results.md'}")
