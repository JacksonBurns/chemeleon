...
class MolDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for atom in mol.GetAtoms():
            atom_idx = ATOM_LIST.index(atom.GetAtomicNum()) if atom.GetAtomicNum() in ATOM_LIST else 0
        ...
        if self.labels is not None:
            ...
            y = torch.tensor(0, dtype=torch.float).view(1, -1)  # Placeholder for test sets without labels
...
def calculate_rmse(y_true, y_pred):
    ...
    if y_true.shape != y_pred.shape:
        logger.warning(f"Shape mismatch in calculate_rmse: y_true {y_true.shape}, y_pred {y_pred.shape}")
...
class MolCLRFineTuner:
    ...
    def _load_pre_trained_weights(self, model):
        try:
            ...
            if 'checkpoint_path' in self.config and os.path.exists(self.config['checkpoint_path']):
                state_dict = torch.load(self.config['checkpoint_path'], map_location=self.device)
                model.load_state_dict(state_dict, strict=False)
            else:
                molclr_path = '/home/akshatz/MolCLR/ckpt/pretrained_gcn/checkpoints/model.pth'
...
def process_benchmark(benchmark_dir, benchmark_name, output_dir, is_classification=False):
    ...
    for seed in RANDOM_SEEDS:
        ...
        # Create datasets
        train_dataset = MolDataset(train_df, smiles_column, target_column, classification=is_classification)
        test_dataset = MolDataset(test_df, smiles_column, classification=is_classification)
        
        # Create data loaders using samplers for train/val split
        train_sampler = SubsetRandomSampler(train_indices)
        val_sampler = SubsetRandomSampler(val_indices)
        
        train_loader = DataLoader(train_dataset, batch_size=32, sampler=train_sampler, num_workers=4, drop_last=False)
        val_loader = DataLoader(train_dataset, batch_size=32, sampler=val_sampler, num_workers=4, drop_last=False)
    ...
    output_path = os.path.join(benchmark_output_dir, f"{benchmark_name}_test_predictions.csv")
    ...
    metadata_path = os.path.join(benchmark_output_dir, f"{benchmark_name}_metadata.json")
...
if __name__ == "__main__":
    ...
    benchmarks_dir = Path(sys.argv[2]) if len(sys.argv) > 2 else Path("/home/akshatz/fastprop_foundation/models/molclr/polaris_benchmarks")
    ...
    with open(summary_path, "w") as summary_file:
        ...
        if os.path.exists(regression_dir):
            ...
            regression_files = [f for f in os.listdir(regression_dir) if f.endswith('_train.csv')]
            
            for train_file in regression_files:
                benchmark_name = train_file.replace('_train.csv', '')
                summary_file.write(f"| {benchmark_name} | Processing | {', '.join(map(str, RANDOM_SEEDS))} |\n")
                process_benchmark(regression_dir, benchmark_name, output_dir, is_classification=False)
                # Update status to completed
                summary_file.seek(summary_file.tell() - len(f"| {benchmark_name} | Processing | {', '.join(map(str, RANDOM_SEEDS))} |\n"))
                summary_file.write(f"| {benchmark_name} | Completed | {', '.join(map(str, RANDOM_SEEDS))} |\n")
        ...
        if os.path.exists(classification_dir):
            ...
            # Find all benchmark train files
            classification_files = [f for f in os.listdir(classification_dir) if f.endswith('_train.csv')]
            
            for train_file in classification_files:
                benchmark_name = train_file.replace('_train.csv', '')
                summary_file.write(f"| {benchmark_name} | Processing | {', '.join(map(str, RANDOM_SEEDS))} |\n")
                process_benchmark(classification_dir, benchmark_name, output_dir, is_classification=True)
                # Update status to completed
                summary_file.seek(summary_file.tell() - len(f"| {benchmark_name} | Processing | {', '.join(map(str, RANDOM_SEEDS))} |\n"))
                summary_file.write(f"| {benchmark_name} | Completed | {', '.join(map(str, RANDOM_SEEDS))} |\n")
    ...
    logger.info(f"All benchmarks processed. Evaluation metadata saved to {eval_metadata_path}")
