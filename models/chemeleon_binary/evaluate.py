...
from chemeleon_binary_fingerprint import CheMeleonBinaryFingerprint
...
class CheMeleonBinaryTransformer(BaseEstimator, TransformerMixin):
    """
    Scikit-learn compatible transformer that computes CheMeleon-Binary fingerprints.
    """

    def __init__(self, device: Union[str, None] = None):
        self.device = device
        self._fingerprinter = CheMeleonBinaryFingerprint(device=self.device)

    def fit(self, X: List[Union[str, Mol]], y=None):
        return self

    def transform(self, X: List[Union[str, Mol]]) -> np.ndarray:
        return self._fingerprinter(X)

    def fit_transform(self, X: List[Union[str, Mol]], y=None, **fit_params) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    output_file.write(
        f"""# CheMeleon-Binary Random Forest Results
...
"""
    ...
    for random_seed in (42, 117, 709, 1701, 9001):
        ...
        for benchmark_name in (
            ...
            pipe = Pipeline(
                [
                    ("smiles2fp", CheMeleonBinaryTransformer()),
                    ("model", model),
                ],
                verbose=True,
            )
