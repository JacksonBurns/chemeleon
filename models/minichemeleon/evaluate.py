...
from chemprop.nn import BinaryClassificationFFN, RegressionFFN, UnscaleTransform, BondMessagePassing
...
"""
    ...
    for random_seed in (42, 117, 709, 1701, 9001):
        ...
        for benchmark_name in (
            ...
            featurizer = SimpleMoleculeMolGraphFeaturizer(atom_featurizer=RIGRAtomFeaturizer(), bond_featurizer=RIGRBondFeaturizer())
            ...
            chemeleon_mp = torch.load("minichemeleon_mp.pt", weights_only=True)
            chemeleon_mp["hyper_params"]["activation"] = torch.nn.GELU()  # <-- chemprop does not support GELU natively
            mp = BondMessagePassing(**chemeleon_mp["hyper_params"])
